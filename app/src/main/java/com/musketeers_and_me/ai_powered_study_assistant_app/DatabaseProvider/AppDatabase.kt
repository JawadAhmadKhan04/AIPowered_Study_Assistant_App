...
class AppDatabase(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    companion object {
        ...
        const val TABLE_GROUP_MESSAGES = "group_messages"
    ...
    override fun onCreate(db: SQLiteDatabase) {
        ...
        db.execSQL("""
            CREATE TABLE $TABLE_GROUP_MESSAGES (
                ...
                type TEXT NOT NULL,
                $COLUMN_PENDING_SYNC INTEGER DEFAULT 1,
                FOREIGN KEY (group_id) REFERENCES $TABLE_GROUP_CHATS(group_id) ON DELETE CASCADE,
                FOREIGN KEY (sender_id) REFERENCES $TABLE_USERS($COLUMN_ID)
            )
        """)
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        ...
        // For now, we'll just drop and recreate all tables
        val tables = arrayOf(
            TABLE_QUIZ_QUESTIONS, TABLE_QUIZZES,
            TABLE_GROUP_MESSAGES, TABLE_GROUP_CHATS, TABLE_GROUP_MEMBERS,
            TABLE_STUDY_GROUPS, TABLE_NOTE_MEMBERS, TABLE_NOTE_CONCEPTS,
            TABLE_NOTE_KEY_POINTS, TABLE_NOTE_TAGS, TABLE_NOTES,
            TABLE_COURSE_MEMBERS, TABLE_COURSES, TABLE_BOOKMARKS,
            TABLE_NOTIFICATIONS, TABLE_USER_PROFILE, TABLE_USER_SETTINGS,
            TABLE_USERS
        )
